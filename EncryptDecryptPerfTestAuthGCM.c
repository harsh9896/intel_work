...
//#endif
//#if 1
    if(1 != EVP_EncryptInit_ex2(*ctx, EVP_aes_256_gcm(), key, iv, NULL))
        handleErrors();
    if(1 != EVP_CIPHER_CTX_ctrl(*ctx, EVP_CTRL_GCM_SET_IVLEN, strlen(iv), NULL))
        handleErrors();
    //     if(1 != EVP_EncryptInit_ex(*ctx, NULL, NULL, key, iv))
...
*/
    //NanoSleep(1000000);
    if(!(*copy_ctx = EVP_CIPHER_CTX_new()))
        handleErrors();
    if(1 != EVP_EncryptInit_ex2(*copy_ctx, EVP_aes_256_gcm(), key, iv, NULL))
        handleErrors();
    if(1 != EVP_CIPHER_CTX_ctrl(*copy_ctx, EVP_CTRL_GCM_SET_IVLEN, strlen(iv), NULL))
        handleErrors();
        // if(1 != EVP_EncryptInit_ex(*copy_ctx, NULL, NULL, key, iv))
        // handleErrors();
//#endif
    //memcpy(ctx, copy_ctx,sizeof(ctx));
    NanoSleep(100000);
    printf("HARSH\n");
    if(1 != EVP_CIPHER_CTX_copy(*copy_ctx, *ctx))
        handleErrors();
}
...
void encrypt_restore_context(EVP_CIPHER_CTX *ctx, EVP_CIPHER_CTX *copy_ctx)
{
   //memcpy(ctx, copy_ctx,sizeof(ctx));
    if(1 != EVP_CIPHER_CTX_copy(ctx, copy_ctx))
       handleErrors();
}
...
int main (int argc,char* argv[])
{
    ...
    //       encrypt_restore_context(encrypt_ctx[iCount%iNoOfUsers], copy_encrypt_ctx[iCount%iNoOfUsers]);
